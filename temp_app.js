...
function App() {
  ...
  const [bodyLanguageScore, setBodyLanguageScore] = useState(100);
  const [gestureTimeline, setGestureTimeline] = useState([]);
  ...
  const drawLandmarks = useCallback((landmarks) => {
    ...
    if (landmarks.pose && landmarks.pose.length > 0) {
      ...
      connections.forEach(([start, end]) => {
        if (landmarks.pose[start] && landmarks.pose[end] &&
            landmarks.pose[start].visibility > 0.5 &&
            landmarks.pose[end].visibility > 0.5) {
          const startX = landmarks.pose[start].x * width;
          const startY = landmarks.pose[start].y * height;
          const endX = landmarks.pose[end].x * width;
          const endY = landmarks.pose[end].y * height;
      ...
      landmarks.hands.forEach((hand) => {
        ...
        handConnections.forEach(([start, end]) => {
          if (hand[start] && hand[end]) {
            const startX = hand[start].x * width;
            const startY = hand[start].y * height;
            const endX = hand[end].x * width;
    ...
    if (landmarks.face && landmarks.face.length > 0) {
      ...
      landmarks.face.forEach((landmark) => {
        const x = landmark.x * width;
        const y = landmark.y * height;
        ctx.beginPath();
  ...
  const captureAndAnalyzeFrame = useCallback(async () => {
    ...
    try {
      ...
      if (response.data.success) {
        ...
        if (response.data.gestures && response.data.gestures.length > 0) {
          setGestures(prev => {
            const newGestures = response.data.gestures;
            const combined = [...prev, ...newGestures];

            // Track all gestures for PDF report
            setAllGesturesList(prevAll => [...prevAll, ...newGestures]);
            ...
            // Update body language score (deduct points for negative gestures)
            const totalNegativeGestures = Object.values(stats).reduce((a, b) => a + b, 0);
            const newBodyLanguageScore = Math.max(100 - (totalNegativeGestures * 3), 0);
            setBodyLanguageScore(newBodyLanguageScore);

            // Update gesture timeline for chart
            setGestureTimeline(prev => {
              const timeElapsed = Math.floor((Date.now() - sessionStartTime.current) / 1000);
              return [...prev, {
                time: timeElapsed,
                gestures: Object.keys(stats).length,
                score: newBodyLanguageScore
              }].slice(-30);
            });
  ...
  const processUploadedVideo = async () => {
    ...
    // Reset session
    await axios.post(`${API_URL}/reset-session`, { session_id: sessionId });
    ...
    const processInterval = setInterval(async () => {
      ...
      setSessionDuration(Math.floor(video.currentTime));
  ...
  const handleExportPDF = () => {
    const sessionData = {
      ...
      speechAnalysis: {
        totalWords: overallScore?.total_words || 0,
        fillerCount: overallScore?.filler_count || 0,
        fillerPercentage: overallScore?.filler_percentage || 0,
        averageWpm: wpmHistory.length > 0 ? Math.round(wpmHistory.reduce((sum, item) => sum + item.wpm, 0) / wpmHistory.length) : 0,
        fillerWordsData
      },
      gestures: {
        total: allGesturesList.length,
        byType: gestureStats,
        list: allGesturesList
      },
    ...
    <div className="App">
          ...
          {isRecording && (
            <div className="session-info">
              <div className="info-card">
                ...
                <span className="info-value">{formatTime(sessionDuration)}</span>
              </div>
              <div className="info-card">
                <span className="info-label">Confidence</span>
                <span className="info-value">{Math.round(confidenceScore)}%</span>
              </div>
              <div className="info-card">
      ...
      <div className="main-container">
                ...
                <>
                  {!isUploadMode ? (
                    <button className="btn btn-start" onClick={startRecording}>
                      <span className="btn-icon">▶</span> Start Live Analysis
                    </button>
                  ) : (
                    <button className="btn btn-start" onClick={processUploadedVideo}>
                      <span className="btn-icon">▶</span> Analyze Uploaded Video
                    </button>
                  )}
                  <label htmlFor="video-upload" className="btn btn-upload">
                    <span className="btn-icon">↑</span> Upload Video
                    <input
                      id="video-upload"
                      type="file"
                      accept="video/*"
                  ...
                  {isUploadMode && uploadedVideo && (
                    <button className="btn btn-clear" onClick={() => {
                      ...
                      <span className="btn-icon">×</span> Clear Video
                    </button>
                  )}
                </>
              ) : (
                <button className="btn btn-stop" onClick={stopRecording}>
                  <span className="btn-icon">■</span> {isProcessingVideo ? 'Processing...' : 'Stop Analysis'}
                </button>
              )}
            </div>
          </div>
        ...
        <div className="right-column">
          ...
          {gestureTimeline.length > 0 && (
            <div className="analytics-panel">
              <h3>
                <span className="panel-icon">●</span>
                Body Language Score Timeline
              </h3>
              <ResponsiveContainer width="100%" height={200}>
                <AreaChart data={gestureTimeline}>
                  <defs>
              ...
              <div className="score-content">
                ...
                <div className="score-details">
                  ...
                  <div className="score-message">
                    ...
                    {overallScore.overall_score >= 60 && overallScore.overall_score < 80 && "Good presentation. Focus on reducing filler words for improvement."}
                    {overallScore.overall_score < 60 && "Needs improvement. Practice gestures and reduce verbal fillers."}
                  </div>
                </div>
              </div>
            </div>
